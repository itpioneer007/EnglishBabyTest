...
TYPE1_RULES = [
    ...
    # 听力判断题：特征词必须含明确判断标识（"相符/一致"单独不算——"选出与录音相符的句子"是选择题！）
    # ★ priority 高于听力选择题：题干常同时含"听录音"(90) 与 "对错"(判断)，判断题特征更具体，须优先
    {"type": "听力判断题", "keywords": ["对错", "√", "×", "正确打", "打勾", "是否相符", "判断相符", "判断对错", "判断正误"],
     "audio": True, "priority": 92},
    # 听力排序题：priority 高于听力选择题——"听录音给句子排序"含"听录音"(90) 但语义是排序
    {"type": "听力排序题", "keywords": ["排序", "排列", "按顺序", "排成", "给下列", "按听到的顺序"],
     "audio": True, "priority": 91},
    # 听力匹配题：priority 高于听力选择题——"听音连线/听录音连线"含"听音"(90) 但语义是匹配
    {"type": "听力匹配题", "keywords": ["听录音匹配", "听一听匹配", "听音匹配", "听录音连线", "听音连线"],
     "audio": True, "priority": 91},
    # 朗读类（口语）
    # ★ 去掉纯"录音"词（"选出与录音相符的句子"含录音却是选择题！）→ 只认朗读动作词
