...
# Windows 控制台默认 GBK 无法输出 emoji(✅❌⚠ 等)，尝试全局切换为 UTF-8。
# 注意: 不能用 io.TextIOWrapper(sys.stdout.buffer, ...) 重新包一层 —— 那种写法新建的对象
# 默认是 closed 状态，会破坏 fileno()，导致 Flask/click 启动横幅打印时报
# "I/O operation on closed file"。正确做法是原地 reconfigure 编码(Python 3.7+)。
...
def detect_screen_resolution(adb_serial: str = ""):
    ...
    # 修正: 原来这里写死成同事的 C:\Users\bunana\... 路径，在其他机器上必崩。
    # 改为自动查找 ADB: 依次尝试 1) PATH 里的 adb  2) 当前用户 WinGet 装的 platform-tools
    # 3) 显式指定的 ANDROID_HOME/ANDROID_SDK_ROOT。任一可用就用。
    try:
        import shutil as _sh
        adb_path = (
            _sh.which("adb")
            or _sh.which("adb.exe")
            or next(
                (str(p) for p in Path(os.path.expanduser("~")).glob(
                    "AppData/Local/Microsoft/WinGet/Packages/Google.PlatformTools*/platform-tools/adb.exe"
                )),
                None,
            )
            or (os.environ.get("ANDROID_HOME") and os.path.join(os.environ["ANDROID_HOME"], "platform-tools", "adb.exe"))
            or (os.environ.get("ANDROID_SDK_ROOT") and os.path.join(os.environ["ANDROID_SDK_ROOT"], "platform-tools", "adb.exe"))
        )
        if not adb_path:
            raise FileNotFoundError("adb.exe 未找到，请安装 Android Platform Tools 或将 adb 加入 PATH")
        cmd = [adb_path]
        if adb_serial:
        ...
        # 兜底: 即便 stdout wrapper 出问题也不让启动崩溃
        try:
...
def run_listening_inspect(version_label: str, unit: int, stage: str, docx_file: str,
    ...
    try:
        ...
        # ★ 记录本次审查的"来源"信息，供实时错题报告表头使用
        _inspection_state["version"] = version_label
        _inspection_state["unit"] = unit
        _inspection_state["stage"] = stage
                ...
                try:
                    ...
                    if script_q:
                        ...
                        record_q_result(
                            ...
                            overall_passed=r.overall_passed,
                            overall_score=round(r.overall_score, 2),
                            stem_reason=r.stem_check.details[0][:300] if r.stem_check.details else "",
                            content_reason=r.content_check.details[0][:300] if r.content_check.details else "",
                            image_reason=r.image_check.details[0][:300] if r.image_check.details else "",
                            answer_reason=r.answer_check.details[0][:300] if r.answer_check.details else "",
                            audio_reason=(r.audio_check.details[0][:300] if getattr(r, 'audio_check', None) and r.audio_check.details else ""),
                            post_error_reason=(r.post_error_check.details[0][:300] if getattr(r, 'post_error_check', None) and r.post_error_check.details else ""),
                            ai_reason=r.content_check.details[0][:300] if r.content_check.details else "",
                            version=version_label, unit=unit, stage=stage,
...
_inspection_state = {
    ...
    "unit": "?",
    "stage": "?",
    "live_report_path": "",    # ★ 实时错题报告(report_live.html)路径
}
...
def api_inspect_question_result():
    ...
    # ★ 每出现一道不通过的题，立即重生成实时错题报告（失败不影响审查主流程）
    if not data.get("overall_passed"):
        try:
            _live_regen_error_report()
        except Exception as e:
            log_msg(f"⚠ 实时错题报告触发失败: {e}", "warning")
...
def api_inspect_reset():
    ...
    _inspection_state["workflow_steps"] = []
    _inspection_state["current_question_idx"] = 0
    _inspection_state["live_report_path"] = ""
    _save_inspection_state()
    # 立即重生成一份空报告，使"查看报告"入口保持可用
    try:
        _live_regen_error_report()
    except Exception:
        pass
...
def _safe_print(*args, **kwargs):
    """后台/无控制台环境下 stdout 可能被关闭，print 会抛 "I/O operation on closed file"。
    包一层，写入失败直接丢弃，保证服务在 headless 场景也能启动。"""
    try:
        print(*args, **kwargs)
    except Exception:
        pass
...
if __name__ == "__main__":
    _safe_print("=" * 50)
    _safe_print("  英语宝模块检测 - Web 控制面板")
    _safe_print("=" * 50)
    _safe_print(f"  项目路径: {PROJECT_ROOT}")
    _safe_print(f"  启动: http://localhost:5000")
    _safe_print("=" * 50)
        # 自动检测屏幕分辨率并缩放坐标
    config = load_config()
