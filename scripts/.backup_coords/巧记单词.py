...
def _find_level_positions(d):
    ...
    for m in re.finditer(
        ...
        w, h = x2-x1, y2-y1
        if 150 < y1 < 500 and 250 < w < 320 and 200 < h < 300:
...
def _answer_loop(d, max_q=20):
    ...
    for _ in range(max_q):
        ...
        if d(text="跳过").exists(timeout=0.5):
            ...
            continue
        # 重新答题（一次答错）
        if d(text="重新答题").exists(timeout=0.5):
            try:
                d(text="重新答题").click()
            except Exception:
                pass
        ...
        if is_fill_letters:
            ...
            for m in re.finditer(
                ...
                if 600 < y1 < 1000 and 180 < w < 210 and 125 < h < 150:
            ...
            if letter_positions:
                ...
                for m in re.finditer(
                    ...
                    x1, y1, x2, y2 = int(m.group(1)), int(m.group(2)), int(m.group(3)), int(m.group(4))
                    w, h = x2-x1, y2-y1
                    if 400 < y1 < 600 and 150 < w < 200 and 100 < h < 130:
                ...
                for bx, by in letter_positions:
                    try:
                        ...
                        pass
                    time.sleep(0.4)
                    if d(text="检查").exists(timeout=0.3):
                        break
                time.sleep(1)
                q += 1
                retry_count = 0
