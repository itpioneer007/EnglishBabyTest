...
def _enter_voice_eval(d, expected_grade="", expected_version=""):
    ...
    # ★ 归一化（全角→半角括号、去空格）避免 '湘少版(2024审定)' 与 App 内
    #   '湘少版（2024审定）' 因括号形态不同被误判为不匹配
    try:
        xml = d.dump_hierarchy()
        _xn = lambda s: (s or '').replace('（', '(').replace('）', ')').replace(' ', '')
        _ver_ok = _xn(expected_version) in _xn(xml)
        _grade_ok = _xn(expected_grade) in _xn(xml)
